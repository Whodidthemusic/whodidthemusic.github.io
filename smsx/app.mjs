...
function render(){if(publicMarketWidget?.parentElement!==document.body)root.after(publicMarketWidget);root.innerHTML=shell(state,ui);const slot=document.querySelector('.tradingview-shell');if(publicMarketWidget&&slot)slot.append(publicMarketWidget);document.title='SMSX — '+(navItems.find(n=>n[0]===ui.view)?.[1]||'Discover');resizeEmbed();}
